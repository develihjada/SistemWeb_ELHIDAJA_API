package com.hidajasac.hidajasac_apis.service.implementacion.rolCon;
...
public class RolImp {
    ...
    public RolResponseDTO saveSer(RolCreateDTO dto) {
        Optional<RolEntity> existing = rolRepository.findByTipoRol(dto.getTipoRol());
        if (existing.isPresent()) {
            throw new ResourceAlreadyExistsException("El rol con tipo '" + dto.getTipoRol() + "' ya existe");
        }
    ...
    public RolResponseDTO desactivateSer(Long id) {
        ...
        if (userRepository.existsByTipoRolIdAndStatusTrue(id)) {
