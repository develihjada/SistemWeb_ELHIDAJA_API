package com.hidajasac.hidajasac_apis.service.implementacion.puestoCon;
...
public class PuestoImp {
    ...
    public PuestoResponseDTO saveSer(PuestoCreateDTO dto) {
        Optional<PuestoEntity> exists = repPuesto.findByTipoPuesto(dto.getTipoPuesto());
        if (exists.isPresent()) {
            throw new ResourceAlreadyExistsException("El puesto con el nombre '" + dto.getTipoPuesto() + "' ya existe");
        }
    ...
    public PuestoResponseDTO updateSer(PuestoUpdateDTO dto, Long id) {
        ...
        // Verificar si el tipoPuesto ya existe y no es el mismo registro
        Optional<PuestoEntity> existing = repPuesto.findByTipoPuesto(dto.getTipoPuesto());
